...
启动 A/B 真实控制后端并打开双机地面站。关闭该地面站或在启动终端按 Ctrl+C，会停止本脚本启动的
两端控制会话。启动脚本本身不会 ARM 或发送运动指令；A/B 都由地面站显式 ARM 后才允许运动。
...
`./scripts/start_ground_dual.sh` 同时查看和控制 A/B（A 四路 RGB，B 三路 RGB）。
